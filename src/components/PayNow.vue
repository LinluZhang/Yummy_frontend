    ...
    <template>
      <main>
          <section class="user">
            ...
            <div class="address_title">
              <label>下单时间：{{cashTime}}</label>
            </div>
            <div class="address_title">
              <label>折后总价：{{totalPrice}}元 <span v-if="totalPrice>=90 && totalPrice<190">（已经满减10元)</span>
                <span v-if="totalPrice>=190">(已经满减20元）</span></label>
            </div>
            <div class="address_title">
              <label>用户级别减免：{{discount}}元</label>
            </div>
            <div class="address_title">
              <label>最终价格：{{totalPrice-discount}}元</label>
            </div>
            <div class="address_title">
              <label>剩余支付时间：{{time}}</label>
            </div>
...
<script>
  ...
  import {url,findMobileUserByEmail, initAddressData, initOrderDetail, findOrderAddressAndTime, getOrderCashTime, changeToCancel, getUserLevel, confirmPay, cancelOrder} from '../data/fetchData'
  ...
  export default {
    ...
    methods: {
      async initData() {
        ...
        await getOrderCashTime(this.$route.params.id).then(res => {
          let temp = res.data;
          this.cashTime = temp[0]['cashTime'];
      ...
      async submit() {
        await confirmPay(this.$route.params.id, localStorage.user, this.restaurantName, this.orderTime, this.address, this.totalPrice, this.discount)//后端生成支付时间
          .then(res => {
            this.$toast({
              icon: 'success',
              message:'付款成功！'
            });
            this.$router.push({path: '/sendingOrder'})
          })
          .catch(err => {
            this.$toast({
              icon: 'fail',
      ...
      async cancel() {
        await cancelOrder(this.$route.params.id)
          .then(res=>{
        ...
        async timeDown() {
          const startTime = new Date(this.cashTime);
          var tempTime = startTime.getTime();
          tempTime += 2*60*1000;//2分钟未支付自动取消订单
          ...
          if (leftTime <= 0) {
            await changeToCancel(this.$route.params.id)
              .then(res => {
