...
<script>
  ...
  import {initOrderRecordData, initOrderDayListData, initRestaurantNameListData,
    initBackOrderRecordDataOrderDayAndRestaurantNameAndCashRange} from '../data/fetchData'
  import {mapState} from 'vuex'
    export default {
      ...
      methods: {
        ...
        async submit() {
          if(this.orderDay ===''||this.orderDay === null){
            this.$toast({
              icon: 'fail',
              message: '请选择下单时间！'
            })
          }else  if(this.restaurantName ==='' || this.restaurantName === null){
            this.$toast({
              icon: 'fail',
              message: '请选择餐厅！'
            })
          }else if(this.cashRange === '' || this.cashRange === null){
            this.$toast({
              icon: 'fail',
              message: '请选择金额区间！'
            })
            ...
            if(searchCashRange === '1'){
              await initBackOrderRecordDataOrderDayAndRestaurantNameAndCashRange(1, searchDay, searchRestaurant, localStorage.user)
                .then(res => {
                  let list = res.data;
                  this.lists = list;
              ...
              await initBackOrderRecordDataOrderDayAndRestaurantNameAndCashRange(2, searchDay, searchRestaurant, localStorage.user)
                .then(res => {
                  let list = res.data;
                  this.lists = list;
                  this.$store.dispatch('initOrderRecordData', {
              ...
              await initBackOrderRecordDataOrderDayAndRestaurantNameAndCashRange(3, searchDay, searchRestaurant, localStorage.user)
                .then(res => {
                  let list = res.data;
                  this.lists = list;
                  this.$store.dispatch('initOrderRecordData', {
              ...
              await initBackOrderRecordDataOrderDayAndRestaurantNameAndCashRange(4, searchDay, searchRestaurant, localStorage.user)
                .then(res => {
                  let list = res.data;
                  this.lists = list;
                  this.$store.dispatch('initOrderRecordData', {
              ...
              await initBackOrderRecordDataOrderDayAndRestaurantNameAndCashRange(5, searchDay, searchRestaurant, localStorage.user)
                .then(res => {
                  let list = res.data;
                  this.lists = list;
                  this.$store.dispatch('initOrderRecordData', {
