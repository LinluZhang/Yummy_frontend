...
<script>
  ...
  import {url,findMobileUserByEmail, initAddressData, initOrderDetail,cancelOrder, getTotalPrice,confirmOrder} from '../data/fetchData'
    export default {
        name: "OrderDetail",
        components:{
        Loading
        ...
        async submit() {
          ...
          if(orderTime!==''&&orderTime!==null){
            if(address!==''&&address!==null){
              await getTotalPrice(this.$route.params.id)
                .then(res=>{
              ...
              await confirmOrder(this.totalPrice, orderTime, address, this.$route.params.id)
                .then(res => {
                  this.$toast({
                    icon: 'success',
                    message: '下单成功，请于2分钟内完成支付！'
                  });
